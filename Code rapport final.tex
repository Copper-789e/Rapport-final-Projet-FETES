...
\begin{document}
	...
	Grâce au fait que la longueur du fil est bien plus grand que son diamètre, on pourra négliger la conduction vers les supports. Le rayonnement thermique est lui aussi négligé.\\
	
	D'après le \textbf{premier principe de la thermodynamique} appliqué au fil chaud, on a donc
	\begin{center}
		$\partial_t U = \Sigma P = P - \dot{Q} $
	\end{center}
	
	\noindent Les expressions pour $P$ la puissance émise par effet Joule et $\dot{Q}$ l'échange temporelle avec le milieu extérieur sont les suivantes :
	\vspace{1mm}
	\begin{equation*}
	...
	\noindent On supposera pour que $T_w$ est identique sur toute la longueur du fil, et nous travaillerons avec le nombre adimensionné de Nusselt, et développant S = "surface latérale du fil chaud" = $d*\pi$, avec d le diamètre du fil chaud :
	\begin{center}
		$\mathrm{Nu} = \frac{h d}{k_f}$; \quad $\dot{Q} = \pi l k_f (T_w - T_0)\,\mathrm{Nu}$
	\end{center}
	avec $k_f$ : coefficient de diffusion thermique.
	\vspace{3mm}
	...
	En prenant l'hypothèse que le régime est \textbf{stationnaire} ($\partial_t U = 0 $), on obtient une première équation fondamentale pour l'expérience:
	\begin{center}
	...
	\section{Loi de King}
	\section{Loi d'évolution de la vitesse}
